...
GroupServiceServer::GroupServiceServer(grpc::Service* service)
    : service_(service) {
}
...
bool GroupServiceServer::Start(const std::string& listen_target) {
    std::lock_guard<std::mutex> lock(mutex_);
    if (service_ == nullptr || listen_target.empty() || server_ != nullptr) {
        return false;
    }
    ...
    builder.AddListeningPort(
        listen_target, grpc::InsecureServerCredentials(), &selected_port
    );
...
bool GroupServiceServer::Running() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return server_ != nullptr && !shutdown_requested_;
}

int GroupServiceServer::SelectedPort() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return selected_port_;
}

std::string GroupServiceServer::BoundTarget() const {
    std::lock_guard<std::mutex> lock(mutex_);
