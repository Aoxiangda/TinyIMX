...
bool ReadPacket(
    ...
    if (reader == nullptr || output == nullptr) return false;
    if (!reader->pending.empty()) {
        *output = std::move(reader->pending.front());
    ...
    for (;;) {
        ...
        if (result.status != tinyimx::DecodeStatus::kOk || result.packets.empty()) {
            return false;
        }
        for (std::size_t i = 1; i < result.packets.size(); ++i) {
            reader->pending.push_back(result.packets[i]);
        }
        *output = result.packets.front();
        return true;
    }
}
...
bool ReadExpected(
    ...
    for (;;) {
        ...
        if (packet.type == type && packet.seq == seq) {
            if (output != nullptr) *output = std::move(packet);
            return true;
        }
    }
}
...
int main(int argc, char* argv[]) {
    if (argc != 6) {
        std::cerr
            << "usage: gateway_group_message_client_demo "
            << "<host> <port> <username> <password> '<json-body>'\n";
        return 2;
    }
    ...
    PacketReader reader;
    if (!Login(fd.Get(), codec, &reader, argv[3], argv[4])) return 1;
    ...
    if (!ReadExpected(
            ...
            codec,
            &reader,
            tinyimx::MessageType::kGroupMessageSendResponse,
            2,
            &response
        )) {
        std::cerr << "group message response not received\n";
        return 1;
    }
