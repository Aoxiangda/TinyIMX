...
std::optional<std::pair<tinyimx::MessageType, tinyimx::MessageType>> OperationTypes(
    ...
    if (operation == "create") return {{MessageType::kCreateGroupRequest, MessageType::kCreateGroupResponse}};
    if (operation == "get") return {{MessageType::kGetGroupRequest, MessageType::kGetGroupResponse}};
    if (operation == "update") return {{MessageType::kUpdateGroupRequest, MessageType::kUpdateGroupResponse}};
    if (operation == "disband") return {{MessageType::kDisbandGroupRequest, MessageType::kDisbandGroupResponse}};
    if (operation == "join") return {{MessageType::kJoinGroupRequest, MessageType::kJoinGroupResponse}};
    if (operation == "leave") return {{MessageType::kLeaveGroupRequest, MessageType::kLeaveGroupResponse}};
    if (operation == "invite") return {{MessageType::kInviteGroupMemberRequest, MessageType::kInviteGroupMemberResponse}};
    if (operation == "kick") return {{MessageType::kKickGroupMemberRequest, MessageType::kKickGroupMemberResponse}};
    if (operation == "role") return {{MessageType::kSetGroupMemberRoleRequest, MessageType::kSetGroupMemberRoleResponse}};
    if (operation == "mute") return {{MessageType::kSetGroupMemberMuteRequest, MessageType::kSetGroupMemberMuteResponse}};
    if (operation == "transfer") return {{MessageType::kTransferGroupOwnershipRequest, MessageType::kTransferGroupOwnershipResponse}};
    if (operation == "members") return {{MessageType::kListGroupMembersRequest, MessageType::kListGroupMembersResponse}};
    if (operation == "my-groups") return {{MessageType::kListMyGroupsRequest, MessageType::kListMyGroupsResponse}};
    return std::nullopt;
}
...
int main(int argc, char* argv[]) {
    if (argc != 7) {
        std::cerr << "usage: gateway_group_client_demo <host> <port> <username> <password> <operation> '<json-body>'\n";
        return 2;
    }
    ...
    if (!ReadExpected(fd.Get(), codec, &reader, types->second, 2, &response)) {
        std::cerr << "group response not received\n";
        return 1;
    }
