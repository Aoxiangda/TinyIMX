...
int main(int argc, char* argv[]) {
    ...
    const tinyimx::Packet login_request =
        MakeLoginRequest(
            "user10001",
            "123456",
            kLoginSeq
    ...
    if (!client.SendPacket(login_request)) {
        std::cerr
    ...
    std::cout
        << "gateway client login validation passed"
        << '\n';
    ...
    const tinyimx::Packet spoof_create_request =
        MakeFriendRequestCreateRequest(
            10004,
            "spoof from_user_id validation",
            kSpoofCreateSeq,
            10004
        );

    if (!client.SendPacket(
            spoof_create_request
        )) {
        std::cerr
            << "send spoof friend request failed"
            << '\n';

        return 1;
    }

    tinyimx::Packet spoof_create_response;

    if (!client.ReceivePacket(
            &spoof_create_response
        )) {
        std::cerr
            << "receive spoof friend request "
            "response failed"
            << '\n';
    ...
    const tinyimx::Packet create_request =
        MakeFriendRequestCreateRequest(
            10004,
            "hello from user10001",
            kCreateSeq
        );
    ...
    tinyimx::Packet create_response;
    ...
    if (!receiver_client.SendPacket(
            no_login_list_request
        )) {
        std::cerr
            << "send no-login list request failed"
            << '\n';
    ...
    tinyimx::Packet
        no_login_list_response;

    if (!receiver_client.ReceivePacket(
            &no_login_list_response
        )) {
        std::cerr
            << "receive no-login list "
            "response failed"
            << '\n';
    ...
    if (!ValidateRejectedResponse(
        ...
        std::cerr
            << "no-login list validation failed"
            << '\n';
    ...
    std::cout
        << "no-login list validation passed"
        << '\n';
    ...
    if (!receiver_client.SendPacket(
        ...
        std::cerr
            << "send receiver login request failed"
            << '\n';
    ...
    tinyimx::Packet
        receiver_login_response;

    if (!receiver_client.ReceivePacket(
            &receiver_login_response
        )) {
        std::cerr
            << "receive receiver login "
            "response failed"
            << '\n';
    ...
    if (!receiver_client.SendPacket(
        ...
        std::cerr
            << "send spoof list request failed"
            << '\n';

        return 1;
    }

    tinyimx::Packet spoof_list_response;

    if (!receiver_client.ReceivePacket(
            &spoof_list_response
        )) {
        std::cerr
            << "receive spoof list response failed"
            << '\n';
    ...
    std::cout
        << "spoof list validation passed"
        << '\n';

    constexpr std::uint32_t
    kListSeq = 7;

    const tinyimx::Packet list_request =
        MakeFriendRequestListRequest(
            20,
            "",
            0,
    ...
    if (!receiver_client.SendPacket(
        ...
        std::cerr
            << "send friend request list failed"
            << '\n';

        return 1;
    }

    tinyimx::Packet list_response;

    if (!receiver_client.ReceivePacket(
            &list_response
        )) {
        std::cerr
            << "receive friend request list "
            "response failed"
            << '\n';
    ...
    std::cout
        ...
        std::cout
        << "gateway friend request "
        "create/list validation passed"
        << '\n';

    constexpr std::uint32_t
    kWrongReceiverAcceptSeq = 8;

    const tinyimx::Packet
        wrong_receiver_accept_request =
            MakeFriendRequestAcceptRequest(
                request_id,
                kWrongReceiverAcceptSeq
            );
    ...
    tinyimx::Packet
        wrong_receiver_accept_response;

    if (!client.ReceivePacket(
            &wrong_receiver_accept_response
        )) {
        std::cerr
            << "receive wrong receiver accept "
            "response failed"
            << '\n';
    ...
    if (!receiver_client.SendPacket(
        ...
        std::cerr
            << "send spoof accept request failed"
            << '\n';
    ...
    tinyimx::Packet
        spoof_accept_response;

    if (!receiver_client.ReceivePacket(
            &spoof_accept_response
        )) {
        std::cerr
            << "receive spoof accept response failed"
            << '\n';
    ...
    if (!ValidateRejectedResponse(
        ...
        std::cerr
            << "spoof accept validation failed"
            << '\n';
    ...
    std::cout
        << "spoof accept validation passed"
        << '\n';
    ...
    const tinyimx::Packet accept_request =
        MakeFriendRequestAcceptRequest(
            request_id,
            kAcceptSeq
        );
    ...
    if (!receiver_client.SendPacket(
        ...
        std::cerr
            << "send friend request accept failed"
            << '\n';

        return 1;
    }

    tinyimx::Packet accept_response;

    if (!receiver_client.ReceivePacket(
            &accept_response
        )) {
        std::cerr
            << "receive friend request accept "
            "response failed"
            << '\n';
    ...
    constexpr std::uint32_t
    kAlreadyAcceptedSeq = 11;

    const tinyimx::Packet
        already_accepted_request =
            MakeFriendRequestAcceptRequest(
                request_id,
                kAlreadyAcceptedSeq
            );
    ...
    if (!receiver_client.SendPacket(
        ...
        std::cerr
            << "send repeated accept request failed"
            << '\n';
    ...
    if (!receiver_client.ReceivePacket(
        ...
        std::cerr
            << "receive repeated accept response "
            "failed"
            << '\n';
    ...
    if (!ValidateAcceptResponse(
        ...
        std::cerr
            << "repeated accept validation failed"
            << '\n';
    ...
    std::cout
        << "repeated accept validation passed"
        << '\n';

    std::cout
        << "gateway friend request accept "
        "validation passed"
        << '\n';

    constexpr std::uint32_t
    kListAfterAcceptSeq = 12;

    const tinyimx::Packet
        list_after_accept_request =
            MakeFriendRequestListRequest(
                20,
                "",
                0,
    ...
    tinyimx::Packet
        list_after_accept_response;

    if (!receiver_client.ReceivePacket(
            &list_after_accept_response
        )) {
        std::cerr
            << "receive list after accept failed"
            << '\n';
    ...
    std::cout
        << "accepted request removed from "
        "pending list validation passed"
        << '\n';

    constexpr std::uint32_t
    kRequesterFriendListSeq = 13;

    const tinyimx::Packet
        requester_friend_list_request =
    ...
    if (!client.SendPacket(
        ...
        std::cerr
            << "send requester friend list failed"
            << '\n';
    ...
    tinyimx::Packet
        requester_friend_list_response;

    if (!client.ReceivePacket(
            &requester_friend_list_response
        )) {
        std::cerr
            << "receive requester friend list failed"
            << '\n';
    ...
    if (!ValidateFriendListContainsUser(
            requester_friend_list_response,
            kRequesterFriendListSeq,
            10001,
            10004
    ...
    std::cout
        << "requester friend list contains "
        "receiver validation passed"
        << '\n';

    constexpr std::uint32_t
    kReceiverFriendListSeq = 14;

    const tinyimx::Packet
        receiver_friend_list_request =
            MakeFriendListRequest(
                20,
    ...
    if (!receiver_client.SendPacket(
        ...
        std::cerr
            << "send receiver friend list failed"
            << '\n';
    ...
    tinyimx::Packet
        receiver_friend_list_response;

    if (!receiver_client.ReceivePacket(
            &receiver_friend_list_response
        )) {
        std::cerr
            << "receive receiver friend list failed"
            << '\n';
    ...
    std::cout
        << "receiver friend list contains "
        "requester validation passed"
        << '\n';
